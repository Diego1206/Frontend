...
const Chat = ({
    ...
    const enviarMensajeYGenerarRespuesta = async (e) => {
        ...
        if (currentPromptValue.toLowerCase().startsWith("/imagen ") || currentPromptValue.toLowerCase().startsWith("/image ")) {
            const imageCommand = currentPromptValue.toLowerCase().startsWith("/imagen ") ? "/imagen " : "/image ";
            const imagePromptText = currentPromptValue.substring(imageCommand.length).trim();
            if (!imagePromptText) {
                 ...
                 <div className="space-y-4">
                     {conversacion.length === 0 && !cargando && !isGeneratingImage ? (
                         ...
                         conversacion.map((mensaje, index) => {
                            const messageKey = `${mensaje.role}-${mensaje.date instanceof Date ? mensaje.date.getTime() : String(mensaje.date)}-${index}-${Math.random().toString(36).substring(7)}`;
